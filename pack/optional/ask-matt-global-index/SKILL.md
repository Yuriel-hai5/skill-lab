---
name: ask-matt
description: 询问当前情境适合哪个技能或流程；它是这台机器上全部已安装 skills 的路由器（覆盖 mattpocock 本包 29 个，以及包外的 finesse-ui、impeccable、ui-ux-pro-max 全家桶、interface-design、taste 系列等）。回答前必须先读 GLOBAL-INDEX.md。
...
本文件下面的 main flow / on-ramps / standalone **只覆盖 mattpocock 本包的 29 个 skill**。
这台机器上还装着包外的 skill，它们不在这张 map 里。

**1. 先确认索引存在且没过期**（一次列目录就能判断）。列出 **skills 目录**下的子目录
（即本 `SKILL.md` 所在目录的父目录——国际版 `~/.workbuddy-ai/skills/`，国内版 `~/.workbuddy/skills/`），
与 [GLOBAL-INDEX.md](GLOBAL-INDEX.md) 记录的数量和名字比对：

- **`GLOBAL-INDEX.md` 不存在**（新装的、还没生成）→ 跑
  `python <本 skill 目录>/scripts/rebuild-index.py` 生成，再继续。
- **存在但对不上**（装了新 skill 或删了旧的）→ 同样跑一次脚本重建，再继续。
- **一致** → 直接读索引。

> 脚本就在本 skill 的 `scripts/` 下，**自包含**，不依赖其他包。
> 若脚本也不存在（只复制了 SKILL.md）→ 跳过索引，退回本文件下面的
> main flow / on-ramps 路由，并告诉用户「全局索引没生成，包外 skill 暂时查不到，
> 跑一次 `rebuild-index.py` 即可」。

新装的未知 skill 会被标成「未分类」，路由时把它当独立 skill 用即可，不必等补 `SOURCE_MAP`。

**2. 读 [GLOBAL-INDEX.md](GLOBAL-INDEX.md)**——它列出本机**全部**已安装 skill、
各自来源包、调用方式（U/M）和用途分组。然后按下面的顺序路由：

3. **设计类任务优先路由到包外。** 本包没有任何 UI/视觉设计 skill。命中下面任一情况，
   直接指向外部包，不要试图用本包的 flow 套：
   - 做任何界面 / 页面 / 组件 / 视觉 → `finesse-ui`（从零，按 brand / product / commerce / h5 四种 register 分流）
   - 改已有界面、评审、局部调优 → `impeccable`（22 个命令：audit / critique / polish / bolder / quieter / distill / colorize / typeset …）
   - 查配色 / 风格 / 字体搭配 / 无障碍规范 / 技术栈实现 → `ui-ux-pro-max`（本地数据库 + `scripts/search.py`）
   - 做 dashboard / admin / SaaS 产品界面 → `interface-design`（方法论）+ `finesse-ui` product register（动手）
   - 做 landing / portfolio / 官网 → `taste-skill`（快）或 `finesse-ui` brand register（重）
4. **"已有界面要升级质感 / 加动效"这类改造任务**（高频场景），按下面拆：
   - **主路线** → `impeccable`：`audit` 先查底子 → `polish` 收尾。想更抢眼用 `bolder`/`delight`/`overdrive`，
     想更安静用 `quieter`/`distill`，动效单独走 `animate`，排版走 `typeset`
   - **质感标准** → `finesse-ui` 的 `references/anti-cheap.md`（廉价黑名单）+ `design-dna.md §1`
     （通用工艺底线：着色中性色、不用纯 `#fff`/`#000`、发丝级半透明边框、着色阴影、对比度下限）
   - **动效方案** → `finesse-ui` 的 `references/motion.md`：EFFECT（看到什么）× ROUTE（怎么实现）两条轴分开，
     10 族效果 / 6 条实现路线 / effect→route 查表——避免为 4 行效果引入 60KB 依赖
   - **GSAP 预设** → `ui-ux-pro-max`：`search.py "<关键词>" --domain gsap`（17 个预设）
   - **别用** `finesse-ui` 的 hero engine：那是 brand register 从零做 hero 用的，改造场景用不上
5. **工程流程类走本包 map。** grill → spec → tickets → implement → review 这类，用下面的 main flow。
6. **两个流程型不要同时主导。** finesse / impeccable 都是端到端流程，同时跑会互相覆盖决策。
   选一个主导，另一个只借用它的检查清单。
