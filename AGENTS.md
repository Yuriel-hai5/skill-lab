...
**skill 是你的工具，不是对话内容。** 说「我接下来怎么做」，不要说「我按哪个 skill 做」。

| ❌ 别说 | ✅ 该说 |
|---|---|
| 「先按 skill 的铁律来：先量化，别猜」 | 「我先量一下截图，别靠感觉猜」 |
| 「我读了 `wpf-visual-bug-triage`」 | （不用说，直接做） |
| 「技能库里有个 `X` 专门管这个」 | （除非要他在两个做法里选，见下） |

**为什么**：报 skill 名是**元话语** —— 它没告诉用户"接下来会发生什么"，
只暴露了你的内部机制。用户不关心你的工具链，只关心结果。

**例外（唯一一处）**：**需要他在两个做法之间选**时，必须说清是什么 ——
不说清他没法选。（例：「这个我可以用 `X` 做（它专门管这类），也可以直接改。你要哪个？」）

> **2026-09-20 实测**：荆丝只说了一句「我看着别扭」（**纯陈述**，没下指令、没提 skill），
> 那边的 AI 回了「**先按 skill 的铁律来：先量化，别猜**」。
> **正例过了**（它确实自己去查了 skill），但他要的是**默默用** ——
> 原话：「你可以**默默思考中使用，别说出来**」。
>
> 另外注意：它说「**铁律**」这个词，是从 `wpf-visual-bug-triage` 的
> 那行标题 `## 铁律：先量化，别猜` 照搬的 ——
