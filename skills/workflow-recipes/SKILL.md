...
**但除此之外，别提 skill 名。** 做法一旦定了，**默默用** ——
说「我接下来怎么做」，不要说「我按哪个 skill 做」。

「**先按 skill 的铁律来**」这种话是**元话语**：它没告诉用户"接下来会发生什么"，
只暴露了你的内部机制。用户不关心工具链，只关心结果。
...
> **例外只有一处**：**需要他在两个做法之间选**时（就是上面表格第一行）——
> 那时必须说清是什么，不说清他没法选。
>
> 2026-09-20 荆丝原话：「你可以**默默思考中使用，别说出来**」。
...
>
> **但别指望模型自己想起来这一步。** 本 skill 自己是「被触发才生效」的，所以它保不了自己 ——
> 2026-09-20 实测：`description` 明明已加载，模型照样没查。
> 真要让**每次**干活都走这一步，得在**项目根 `AGENTS.md`** 里写死一段（每次对话自动注入，
> 不依赖模型主动回忆）。现成可粘贴的 block → [11-setup-docs](references/11-setup-docs.md) 的 **配方 G**。
> 所以：如果你发现用户这个项目**没有**那段，**主动提一次**。
